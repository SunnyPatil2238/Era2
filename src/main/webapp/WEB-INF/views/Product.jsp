...
function printData()
{
   ...
   var ProductPrice=document.getElementById("Selling_Price").value;
   
   //var barcode=document.getElementById("barcode").innerHTML;
   var barcode=document.getElementById("svgContent");
   newWin= window.open("<html>");
   newWin.document.write("<style>");
   newWin.document.write(".split {height: 100%;width: 48%;position: fixed;}");
   newWin.document.write(".left {  left: 5;}");
   newWin.document.write(".right {right: 0;}");
   newWin.document.write("</style>");
   newWin.document.write("</head>");
   newWin.document.write("<body>");

   //newWin.document.write("<h3 align='center'>"+header+"</h3>");
  
   newWin.document.write("<div class='split left '>");
   newWin.document.write("<h1>"+Product);
   newWin.document.write(" <br/> MRP : "+ProductMRP);
   newWin.document.write(" <br/> Price : "+ProductPrice+"</h1>");
   newWin.document.write(barcode.outerHTML);
   newWin.document.write("</div>");
   
   newWin.document.write("<div class='split right '>");
   newWin.document.write("<h1>"+Product);
   newWin.document.write(" <br/> MRP : "+ProductMRP);
   newWin.document.write(" <br/> Price : "+ProductPrice+"</h1>");
   newWin.document.write(barcode.outerHTML);
		...
		<c:choose>
			<c:when test="${(mode== 'Product_View')}">
				...
				<table class="table table-hover">
					...
					<tbody id="myTable">
						<c:forEach var="Product" items="${AllProd}">
							<tr >
							
								<td>${Product.getProductId()}</td>
								<td>${Product.getProductName()}</td>
								<td>${Product.getQuantity()}</td>
								<td>${Product.getProductMRP()}</td>
								<td>${Product.getProductBuyingPrice()}</td>
								<td>${Product.getProductSellingPrice()}</td>
								<td><a href="prodItemVew?id=${Product.getProductId()}"><span
			...
			<c:when test="${(mode=='Product_Item_Add')}">
			...
			<%-- 		<div class="form-group col-lg-12">
					...
					<div class="form-group col-lg-8">
							<label for="Name">Name(वस्तूचे नाव)  </label> <input type="text"
								<%-- value="${Customer.get().getCustFirstName()}" --%>
								required="required" class="form-control" id="Name" name="Name" />
					</div>
					
						<div class="form-group col-lg-4">
							<label for="Barcode">Barcode (बारकोड )</label> <input type="text"
								<%-- value="${Customer.get().getCustFirstName()}" --%>
						...
						if(cat==="Beverages (पेय)")
						{
							
							document.getElementById("Category_Info").value = " कॉफी /चहा /जुस /सोडा.. "
							
						}
						else if(cat==="Bakery (बेकरी )")
							{
							
							document.getElementById("Category_Info").value = " सँडविच रोल ... " 
							}
						
						else if(cat==="Jarred Goods (भांडे माल)"
								)
						{
						document.getElementById("Category_Info").value = "भाज्या /सौस / केचअप "
						}
						else if(cat==="Dairy(दुग्धजन्य )")
						{
						document.getElementById("Category_Info").value= "चीझ /अंडी/ दूध/ दही "
						}
						else if(cat==="Baking Goods(भाजलेले )")
						{
						document.getElementById("Category_Info").value = "तृणधान्ये/साखर /पीठ / पास्ता ... "
						}
						else if(cat==="Frozen Foods (गोठलेले)")
						{
						document.getElementById("Category_Info").value = "भाज्या /आईस क्रीम ... "
						}
						else if(cat==="Produce(उत्पादन)")
						{
						document.getElementById("Category_Info").value= "फळ /भाज्या .... "
						}
						else if(cat==="Cleaners (साफ सफाई )")
						{
						document.getElementById("Category_Info").value= "धुण्याची पावडर / भांडी धुण्याचे लिक्विड /पावडर ... "
						}
						else if(cat==="Paper Goods (कागदी वस्तू)")
						{
						document.getElementById("Category_Info").value ="paper towels, toilet paper, aluminum foil, sandwich bags"
						}
						else if(cat==="Personal Care (वैयक्तिक काळजी / मेकअप )")
						{
						document.getElementById("Category_Info").value=  "शाम्पू / साबण /हॅन्ड वॉश /दाढी क्रीम ... "
						}
						else if(cat==="Other (इतर )")
						{
						document.getElementById("Category_Info").value= "लहान मुलांचे साहित्य / पाळीव प्राण्यांचे साहित्य / बॅटरी /ग्रीटिंग कार्ड्स "
						}
						else if(cat==="--Select--")
						{
						document.getElementById("Category_Info").value =""
						}
						}
						</script>

						<div class="form-group col-lg-4">
							<label for="Category">Category (प्रकार )</label>
							 <select class="form-control" id="Category" name ="Category" onchange="categoryInfo(this.value)" >
							<option>--Select--(निवडा )</option>
							<option>Beverages (पेय)</option>
							<option>Bakery (बेकरी )</option>
							<option>Jarred Goods (भांडे माल)</option>
							<option>Dairy(दुग्धजन्य )</option>
							<option>Baking Goods(भाजलेले )</option>
							<option>Frozen Foods (गोठलेले)</option>
							<option>Produce(उत्पादन)</option>
							<option>Cleaners (साफ सफाई )</option>
							<option>Paper Goods (कागदी वस्तू) </option>
							<option>Personal Care (वैयक्तिक काळजी / मेकअप )</option>
							<option>Other (इतर )</option>
						</select>
					</div>
					
					<div class="form-group col-lg-4">
							<label for="Category_Info">Category Info (उदाहरणार्थ) </label> <input type="text"
								<%-- value="${Customer.get().getCustFirstName()}" --%>
								class="form-control" id="Category_Info" name="Category_Info" readonly />
					</div>
					
					
					
					<div class="form-group col-lg-4">
							<label for="Quantity">Quantity (संख्या) </label> <input type="number"
								<%-- value="${Customer.get().getCustFirstName()}" --%>
								required="required" class="form-control" id="Quantity" name="Quantity" />
					</div>
						
					
					<div class="form-group col-lg-3">
							<label for="MRP" class="fa fa-rupee">MRP (पूर्ण किंमत) </label> <input type="number" step ="any"
								<%-- value="${Customer.get().getCustFirstName()}" --%>
								required="required" class="form-control" id="MRP" name="MRP" />
					</div>
					
					<div class="form-group col-lg-3">
							<label for="Buying_Price" class="fa fa-rupee">Buying Price (खरेदी किंमत) </label> <input type="number" step ="any"
								<%-- value="${Customer.get().getCustFirstName()}" --%>
								required="required" class="form-control" id="Buying_Price" name="Buying_Price" onchange="calProfit()" />
					</div>
					<div class="form-group col-lg-3">
							<label for="Selling_Price" class="fa fa-rupee">Selling Price (विक्री किंमत) </label> <input type="number" step ="any"
								<%-- value="${Customer.get().getCustFirstName()}" --%>
								required="required" class="form-control" id="Selling_Price" name="Selling_Price" onchange="calProfit()" />
					</div>
					<script>
					function calProfit()
					{
						if(document.getElementById("Selling_Price").value.length!==0  && document.getElementById("Buying_Price").value.length!==0 )
							{
							document.getElementById("profit").value=(document.getElementById("Selling_Price").value-document.getElementById("Buying_Price").value).toFixed(2);
							}
					}
					
					</script>
					<div class="form-group col-lg-3">
							<label for="profit" class="fa fa-rupee">Profit (नफा) </label> <input type="number" step ="any"
								<%-- value="${Customer.get().getCustFirstName()}" --%>
								readonly class="form-control" id="profit" name="profit" />
					</div>
					
					<div class="form-group col-lg-3">
							<label for="Damage_Count" class="fa fa-chain-broken">Damage count (नुकसान संख्या) </label> <input type="number" step ="any"
								<%-- value="${Customer.get().getCustFirstName()}" --%>
								class="form-control" id="Damage_Count" name="Damage_Count" />
					</div>
					
					<div class="form-group col-lg-3">
							<label for="sGst" class="fa fa-rupee">sGST (राज्य जीएसटी)  </label> <input type="number" step ="any"
								<%-- value="${Customer.get().getCustFirstName()}" --%>
								class="form-control" id="sGst" name="sGst" />
					</div>
					<div class="form-group col-lg-3">
							<label for="cGst" class="fa fa-rupee">cGST (केंद्रीय जीएसटी)  </label> <input type="number" step ="any"
								<%-- value="${Customer.get().getCustFirstName()}" --%>
								class="form-control" id="cGst" name="cGst" />
					</div>
					
					<div class="form-group col-lg-3">
							<label for="cGst" class="fa fa-rupee">iGST (आंतरराज्जीय जीएसटी)  </label> <input type="number" step ="any"
								<%-- value="${Customer.get().getCustFirstName()}" --%>
								class="form-control" id="iGst" name="iGst" />
					</div>
					
					<div class="form-group col-lg-6">
							<label for="FreeProductid1" class="fa fa-free-code-camp">Free Product1 (विनामूल्य वस्तू १)</label> <input type="number" step ="any"
								<%-- value="${Customer.get().getCustFirstName()}" --%>
								class="form-control" id="FreeProductid1" name="FreeProductid1" />
					</div>	
					
					<div class="form-group col-lg-6">
							<label for="FreeProductid2" class="fa fa-free-code-camp">Free Product2 (विनामूल्य वस्तू २)</label> <input type="number" step ="any"
								<%-- value="${Customer.get().getCustFirstName()}" --%>
								class="form-control" id="FreeProductid2" name="FreeProductid2" />
					</div>	
					...
					<button type="submit" class="btn"><i class="fa fa-folder"></i>Submit</button>
			...
			<c:when test="${(mode=='Product_Item_Vew')}">
				<%-- <c:url var="custmodurl" value="custMod}"></c:url> --%>
				<form action="AllProducts" method="post">
					...
					<div class="form-group col-lg-5">
						...
						<div class="form-group col-lg-3">
							<label for="Barcode">Barcode (बारकोड )</label> <input type="text"
								value="${Product.getProductBarcode()}" readonly
								class="form-control" id="Barcode" name="Barcode" />
						</div>
						
						<div class="form-group col-lg-4">
							<label for="Category">Category (प्रकार )</label> <input type="text"
					...
					<div class="form-group col-lg-4">
							<label for="Category_Info">Category Info (उदाहरणार्थ) </label> <input type="text"
								value="${Product.getProductCategoryDesc()}"
								class="form-control" id="Category_Info" name="Category_Info" readonly />
					</div>
					...
					<div class="form-group col-lg-3">
							<label for="MRP" class="fa fa-rupee">MRP (पूर्ण किंमत) </label> <input type="number" step ="any"
								value="${Product.getProductMRP()}" readonly
								required="required" class="form-control" id="MRP" name="MRP" />
					</div>
					
					<div class="form-group col-lg-3">
							<label for="Buying_Price" class="fa fa-rupee">Buying Price (खरेदी किंमत) </label> <input type="number" step ="any"
								value="${Product.getProductBuyingPrice()}" readonly
								required="required" class="form-control" id="Buying_Price" name="Buying_Price" onchange="calProfit()" />
					</div>
					<div class="form-group col-lg-3">
							<label for="Selling_Price" class="fa fa-rupee">Selling Price (विक्री किंमत) </label> <input type="number" step ="any"
								value="${Product.getProductSellingPrice()}" readonly
								required="required" class="form-control" id="Selling_Price" name="Selling_Price" onchange="calProfit()" />
					</div>
					
					<div class="form-group col-lg-3">
							<label for="profit" class="fa fa-rupee">Profit (नफा) </label> <input type="number" step ="any"
								value="${Product.getProductProfit()}" readonly
								readonly class="form-control" id="profit" name="profit" />
					</div>
					
					<div class="form-group col-lg-3">
							<label for="Damage_Count" class="fa fa-chain-broken">Damage count (नुकसान संख्या) </label> <input type="number" step ="any"
								value="${Product.getProductDamageCount()}" readonly
								class="form-control" id="Damage_Count" name="Damage_Count" />
					</div>
					
					<div class="form-group col-lg-3">
							<label for="sGst" class="fa fa-rupee">sGST (राज्य जीएसटी)  </label> <input type="number" step ="any"
								value="${Product.getProductsGst()}" readonly
								class="form-control" id="sGst" name="sGst" />
					</div>
					<div class="form-group col-lg-3">
							<label for="cGst" class="fa fa-rupee">cGST (केंद्रीय जीएसटी)  </label> <input type="number" step ="any"
								value="${Product.getProductcGst()}" readonly
								class="form-control" id="cGst" name="cGst" />
					</div>
					
					<div class="form-group col-lg-3">
							<label for="cGst" class="fa fa-rupee">iGST (आंतरराज्जीय जीएसटी)  </label> <input type="number" step ="any"
								value="${Product.getProductiGst()}" readonly
								class="form-control" id="iGst" name="iGst" />
					</div>
					
					<div class="form-group col-lg-6">
							<label for="FreeProductid1" class="fa fa-free-code-camp">Free Product1 (विनामूल्य वस्तू १)</label> <input type="number" step ="any"
								value="${Product.getProductFreeProductid1()}" readonly
								class="form-control" id="FreeProductid1" name="FreeProductid1" />
					</div>	
					
					<div class="form-group col-lg-6">
							<label for="FreeProductid2" class="fa fa-free-code-camp">Free Product2 (विनामूल्य वस्तू २)</label> <input type="number" step ="any"
								value="${Product.getProductFreeProductid2()}" readonly
								class="form-control" id="FreeProductid2" name="FreeProductid2" />
					</div>	
					...
					<a href="updProd?id=${Product.getProductId()}" class="btn btn-primary btn-md " role="button" aria-pressed="true">Edit (सुधारणे)
					</a>
					<a href="delProd?id=${Product.getProductId()}" class="btn btn-danger btn-md " role="button" aria-pressed="true" onclick="return confirm('Are you sure you want to delete this item (तुम्हाला नक्की डिलीट करायचे आहे का)?');" >Delete (हटवा)
					</a>
					<a href="javascript:getbarcode()" class="btn btn-danger btn-md " role="button" aria-pressed="true" >Generate Barcode (बारकोड बनवा)
					</a>
					<a href="javascript:printData()" class="btn btn-danger btn-md " role="button" aria-pressed="true" >Print Barcode (बारकोड प्रिंट)
					</a>
					</div>
				</form>
			...
			<c:when test="${(mode=='Product_Item_Mod')}">
				
				<form action="ProdModify" method="post">
				...
				<div class="form-group col-lg-4">
					...
					<div class="form-group col-lg-4">
						...
						<div class="form-group col-lg-4">
							<label for="Barcode">Barcode (बारकोड )</label> <input type="text"
								value="${Product.getProductBarcode()}" 
								class="form-control" id="Barcode" name="Barcode" />
						</div>
						<script>
						function categoryInfo(cat){
						...
						<div class="form-group col-lg-4">
							<label for="Category">Category (प्रकार )</label>
							 <select class="form-control" id="Category" name ="Category" onchange="categoryInfo(this.value)" >
							 <!-- <option>${Product.getProductCategory()}</option>-->
							 <option>${fn:replace(Product.getProductCategory(), "\"", "\\\"")}</option>
							<option>--Select--(निवडा )</option>
							<option>Beverages (पेय)</option>
							<option>Bakery (बेकरी )</option>
							<option>Jarred Goods (भांडे माल)</option>
					...
					<div class="form-group col-lg-3">
							<label for="MRP" class="fa fa-rupee">MRP (पूर्ण किंमत) </label> <input type="number" step ="any"
								value="${Product.getProductMRP()}"
								required="required" class="form-control" id="MRP" name="MRP" />
					</div>
					
					<div class="form-group col-lg-3">
							<label for="Buying_Price" class="fa fa-rupee">Buying Price (खरेदी किंमत) </label> <input type="number" step ="any"
								value="${Product.getProductBuyingPrice()}"
								required="required" class="form-control" id="Buying_Price" name="Buying_Price" onchange="calProfit()" />
					</div>
					<div class="form-group col-lg-3">
							<label for="Selling_Price" class="fa fa-rupee">Selling Price (विक्री किंमत) </label> <input type="number" step ="any"
								value="${Product.getProductSellingPrice()}"
								required="required" class="form-control" id="Selling_Price" name="Selling_Price" onchange="calProfit()" />
					</div>
					<script>
					function calProfit()
					...
					<div class="form-group col-lg-3">
							<label for="profit" class="fa fa-rupee">Profit (नफा) </label> <input type="number" step ="any"
								value="${Product.getProductProfit()}" 
								readonly class="form-control" id="profit" name="profit" />
					</div>
					
					<div class="form-group col-lg-3">
							<label for="Damage_Count" class="fa fa-chain-broken">Damage count (नुकसान संख्या) </label> <input type="number" step ="any"
								value="${Product.getProductDamageCount()}" 
								class="form-control" id="Damage_Count" name="Damage_Count" />
					</div>
					
					<div class="form-group col-lg-3">
							<label for="sGst" class="fa fa-rupee">sGST (राज्य जीएसटी)  </label> <input type="number" step ="any"
								value="${Product.getProductsGst()}" 
								class="form-control" id="sGst" name="sGst" />
					</div>
					<div class="form-group col-lg-3">
							<label for="cGst" class="fa fa-rupee">cGST (केंद्रीय जीएसटी)  </label> <input type="number" step ="any"
								value="${Product.getProductcGst()}"
								class="form-control" id="cGst" name="cGst" />
					</div>
					
					<div class="form-group col-lg-3">
							<label for="cGst" class="fa fa-rupee">iGST (आंतरराज्जीय जीएसटी)  </label> <input type="number" step ="any"
								value="${Product.getProductiGst()}"
								class="form-control" id="iGst" name="iGst" />
					</div>
					
					<div class="form-group col-lg-6">
							<label for="FreeProductid1" class="fa fa-free-code-camp">Free Product1 (विनामूल्य वस्तू १)</label> <input type="number" step ="any"
								value="${Product.getProductFreeProductid1()}"
								class="form-control" id="FreeProductid1" name="FreeProductid1" />
					</div>	
					
					<div class="form-group col-lg-6">
							<label for="FreeProductid2" class="fa fa-free-code-camp">Free Product2 (विनामूल्य वस्तू २)</label> <input type="number" step ="any"
								value="${Product.getProductFreeProductid2()}"
								class="form-control" id="FreeProductid2" name="FreeProductid2" />
					</div>	
